...
const Contact = () => {
  ...
  const sendEmail = (e) => {
    e.preventDefault();
    if(document.getElementById('from_name').value == "" || document.getElementById('from_email').value == "" || document.getElementById('message').value == "") {
      e.preventDefault()
    } else {
      emailjs
  ...
  return (
      ...
      <section className="text-white 2xl:text-5xl  w-full h-screen flex items-center justify-between flex-col" id="contact">
        <div className="2xl:w-8/12 2xl:h-3/6 xl:w-10/12 lg:w-11/12 md:w-full md:px-5 mt-40 max-sm:mt-28 flex flex-col 2xl:gap-5 md:relative max-sm:relative max-sm:w-9/12 ">
          ...
          <form className="w-full h-full flex gap-10 max-sm:flex-col max-sm:gap-2" ref={form} onSubmit={sendEmail}>
            <div className="w-full h-full flex flex-col gap-4 max-sm:gap-2">
              ...
              <div className="flex flex-col gap-2">
                ...
                <input type="email" name="from_email" id="from_email" className="w-full text-base max-sm:text-xs p-2 rounded focus:ring focus:ring-lime-300 focus:border-none shadow-[0_0_5px_5px_rgba(132,204,22,1)] shadow-lime-500 focus:text-lime-300 focus:bg-black text-black" placeholder="Enter email"/>
              </div>
              <div className="w-full h-16 flex gap-5 text-base max-sm:text-xs justify-center 2xl:static lg:static md:absolute xl:static md:-bottom-16 md:items-center -right-[45px] max-sm:absolute max-sm:-bottom-16 max-sm:gap-2 max-sm:w-screen max-sm:items-center md:-right-0 md:-bottom-18 max-sm:px-1 relative md:margin-0 max-sm:margin-0 max-sm:-translate-x-1/2 max-sm:left-1/2">
                ...
                <div className="flex items-center max-sm:text-xs text-lime-300 max-sm:w-full justify-center max-sm:absolute -bottom-5 ">Email: robert.ramos07281021@gmail.com</div>
              </div>
            ...
            <div className="w-full h-full flex flex-col gap-2">
              ...
              <textarea name="message" id="message" className=" resize-none w-full text-base max-sm:text-xs p-2 rounded focus:ring focus:ring-lime-300 focus:border-none shadow-[0_0_5px_5px_rgba(132,204,22,1)] shadow-lime-500 focus:text-lime-300 focus:bg-black text-black md:h-56 max-sm:h-24" placeholder="Message"></textarea>
        ...
        <footer className="w-full h-16 max-sm:h-16 2xl:h-16 bg-black flex justify-center p-0 items-center shadow-[0px_-2px_7px_7px_rgba(132,204,22,1)] gap-5 max-xl:text-xl xl:h-16 lg:h-10 lg:text-xl md:h-10  max-sm:gap-2">
